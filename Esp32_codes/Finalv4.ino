    ...
    */
    ...
    switch (state) {
      ...
      // Internet check with 10s active try, then 1-minute backoff
      enum CheckPhase { PHASE_CHECKING = 0, PHASE_BACKOFF = 1 };
      ...
      static unsigned long lastStateSeen = 0;
      const char spinnerChk[4] = {'|','/','-','\\'};

      // Initialize phase on first entry or when stateTs changes
      if (lastStateSeen != stateTs) {
        lastStateSeen = stateTs;
      ...
      if (phase == PHASE_CHECKING) {
        // Try reaching API; if not yet reachable, animate for up to 10s
        if (!isApiReachable()) {
          ...
          // After 10 seconds, enter backoff mode for 1 minute
          if (millis() - phaseStart > 10000UL) {
            phase = PHASE_BACKOFF;
            backoffUntil = millis() + 60000UL;
            lcd.setCursor(0, 0);
            lcd.print("No internet     ");
            lcd.setCursor(0, 1);
            lcd.print("Retry in 1 min  ");
          }
          break; // stay in FLUSHING_OFFLINE
        }
        // Internet is reachable -> continue to sending animation and flush
      } else if (phase == PHASE_BACKOFF) {
        // Wait until the backoff elapses before trying again
        if ((long)(millis() - backoffUntil) < 0) {
          // Optional subtle dot animation on line 1 during backoff
          ...
          if (millis() - lastDot > 500) {
            ...
            for (uint8_t i = dots; i < 3; i++) lcd.print(" ");
          }
          break; // still backing off
        }
        // Backoff elapsed -> reset to checking
        phase = PHASE_CHECKING;
        phaseStart = millis();
        lastAnimChk = 0;
        spIdxChk = 0;
        // fall-through to attempt again this loop
      }

      // If we are here, internet is reachable -> show sending animation and flush
      ...
      if (millis() - lastAnim > 200) {
        ...
        // Line 1: moving block (reserve col 15 for badge)
        lcd.setCursor(0, 1);
        lcd.print("                ");
        uint8_t pos = animPos % 15; // 0..14 only
        lcd.setCursor(pos, 1);
        lcd.print((char)255);
        updateWifiBadge();
      }
    ...
    // Define custom chars for WiFi badge (char 0) and cross (char 1)
    uint8_t wifiGlyph[8] = {
      0b00000,
      0b00000,
      0b00100,
    ...
    // Brief WiFi toast (500 ms) shown only when idle
    void showWifiToast(const char* msg) {
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print(msg);
    ...
    // Bottom-right WiFi/Internet badge: column 15, row 1
    // Shows WiFi glyph (char 0) when internet reachable; cross (char 1) otherwise
    void updateWifiBadge() {
      lcd.setCursor(15, 1);
